...
from services.evaluation_models import EvaluationBatchRequest, EvaluationTestCaseRequest
...
evaluation_service = EvaluationService(
    chat_service=chat_service, repository=evaluation_repository
)
...
@app.post(
    ...
    responses={
        400: {"description": "Solicitud invalida para evaluar"},
        500: {"description": "Error interno durante evaluacion"},
    },
)
def evaluation_test_case(payload: EvaluationTestCaseRequest) -> Dict[str, Any]:
    try:
        return evaluation_service.run_test_case(payload)
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
    except Exception as exc:
        raise HTTPException(
            status_code=500, detail=f"Error evaluando caso: {exc}") from exc


@app.post(
    "/evaluation/batch",
    responses={
        400: {"description": "Solicitud invalida para batch"},
        500: {"description": "Error interno durante batch"},
    },
)
def evaluation_batch(payload: EvaluationBatchRequest) -> Dict[str, Any]:
    try:
        return evaluation_service.run_batch(payload.cases)
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
    except Exception as exc:
        raise HTTPException(
            status_code=500, detail=f"Error evaluando batch: {exc}") from exc


@app.get(
    "/evaluation/latest",
    responses={500: {"description": "Error al listar evaluaciones"}},
)
def evaluation_latest(
    limit: Optional[int] = Query(default=None, ge=1, le=200),
) -> Dict[str, Any]:
    try:
        results = evaluation_repository.get_latest_results(limit=limit)
        return {
            "status": "ok",
            "count": len(results),
            "results": results,
        ...
        raise HTTPException(
            status_code=500, detail=f"Error listando evaluaciones: {exc}") from exc


@app.get(
    "/evaluation/summary",
    responses={500: {"description": "Error al resumir evaluaciones"}},
)
def evaluation_summary() -> Dict[str, Any]:
    try:
        summary = evaluation_repository.get_summary_by_course()
        return {
            "status": "ok",
            "summary": summary,
        }
        ...
        raise HTTPException(
            status_code=500, detail=f"Error generando resumen: {exc}") from exc
